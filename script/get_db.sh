...
# 원격 DB에서 덤프를 받아서 로컬 DB로 복원하는 스크립트
# 사용법: ./get_db.sh [--overwrite|--append]
...
if [ "$1" == "--append" ]; then
    ...
    echo "복원 모드를 선택하세요:"
    echo "=========================================="
    echo "1) 덮어쓰기 (기존 로컬 DB를 완전히 교체)"
    echo "2) 추가 (기존 로컬 DB에 데이터 추가)"
    echo "=========================================="
    ...
    echo "  --overwrite: 로컬 DB를 완전히 덮어씁니다"
    echo "  --append: 로컬 DB에 데이터를 추가합니다"
    echo ""
...
# 1. 원격 DB에서 덤프 생성
echo "📥 원격 DB에서 덤프 생성 중..."

# docker-compose가 실행 중인지 확인 (컨테이너 내부의 pg_dump 사용)
...
# docker-compose 컨테이너 내부의 pg_dump를 사용하여 원격 DB에 접속 (버전 불일치 해결)
docker-compose -f "${DOCKER_COMPOSE_FILE}" exec -T postgres \
    env PGPASSWORD="${REMOTE_PASSWORD}" pg_dump \
    -h "${REMOTE_HOST}" \
    -U "${REMOTE_USER}" \
    -d "${REMOTE_DB}" \
    --no-owner \
...
if [ "${MODE}" == "overwrite" ]; then
    echo "🔄 덮어쓰기 모드: 기존 데이터베이스를 삭제하고 복원합니다..."
